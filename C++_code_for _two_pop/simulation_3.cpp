...
List simulation(NumericVector p0, NumericVector q0, IntegerVector N, IntegerVector M, double c, NumericMatrix migration){
    ...
    */
    ...
    for (int i = 1; i < t; ++i){
        ...
        IntegerVector pop_1_migrates = {};
        if (size1[i - 1] < v.length()){
            pop_1_migrates = temp1[Range(size1[i - 1], v.length() - 1)];
        }
        ...
        IntegerVector pop_2_migrates = {};
        if (size3[i - 1] < v.length()){
            pop_2_migrates = temp2[Range(size3[i - 1], v.length() - 1)];
        }
