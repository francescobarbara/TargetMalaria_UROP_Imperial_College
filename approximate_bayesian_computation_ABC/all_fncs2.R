...
df_and_plots = function(full_mat, values_per_layer, simulated_values_per_layer, test_number = 666, save_df = FALSE){
  ...
  df$col <- rbPal(1000)[as.numeric(cut(log(df$mse),breaks = 1000))]
...
sequential_mc = function(real_values, recombination_rates, values_per_layer, 
  ...
  upperbound_pop1 = 3*median(upperbound_pop1)
  upperbound_pop2 = 3*median(upperbound_pop2)
  ...
  estimates_previous_layer[,3] = runif(values_per_layer, 0, 0.5)
  ...
  while((tree_depth_count <= max_depth) & (improvement > improvement_threshold)){
    ...
    for (i in 1:simulated_values_per_layer){
      ...
      vals = estimates_previous_layer[index,c(1,2,3)] + 2*standard_deviations * rnorm(3)
      
      while ( !( (vals[1] > 0) & (vals[2] > 0) & (vals[3] > 0) & (vals[3] <= 0.5)  ) ){
        
        vals = estimates_previous_layer[index,c(1,2,3)] + c(2,2,4)*standard_deviations * rnorm(3)  
      }
